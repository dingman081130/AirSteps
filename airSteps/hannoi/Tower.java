...
public class Tower extends JPanel{
		...
		public void removeDisk(){
			if(pointA !=null){
				for(int i =0;i<pointA.length;i++){
					...
					pointA[i].removeDisc(pointB[i].getDiscOnPoint(),this);
					pointA[i].removeDisc(pointC[i].getDiscOnPoint(),this);
				}
			}
		}
		public void paintComponent(Graphics g){
			...
			y1 =pointC[amountOfDisc-1].getY()+discHeight/2;
			int length =x2-x1,height =6;
			g.fillRect(x1,y1,length,height);
			int size =5;
			for(int i =0;i<pointA.length;i++){
				g.fillOval(pointA[i].getX()-size/2,pointA[i].getY()-size/2,size,size);
				g.fillOval(pointB[i].getX()-size/2,pointA[i].getY()-size/2,size,size);
				g.fillOval(pointC[i].getX()-size/2,pointA[i].getY()-size/2,size,size);
			}
